...
export async function searchSearxng({ query, limit, safeSearch, signal, source = 'web', categories = 'general', pages = 1 }) {
  ...
  const raw = bodies.flatMap((body) => body.results ?? []).filter((item) => {
    ...
    seen.add(item.url);
    return source === 'web' || matchesSource(item.url, source);
  }).slice(0, limit);

  return {
    configured: true,
    results: raw.map((item, index) => {
      const origin = source === 'web' ? sourceFromUrl(item.url, 'web') : source;
      return {
