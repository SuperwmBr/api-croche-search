...
export async function searchSearxng({ query, limit, safeSearch, signal }) {
  ...
  return { configured: true, results: (body.results ?? []).slice(0, limit).map((item, index) => ({
    id: `searxng:${Buffer.from(item.url).toString('base64url').slice(0, 24)}`, externalId: item.url, type: 'artigo', origin: item.engine ?? 'web',
    ...
    rankingSignals: { textualRelevance: Math.max(0.4, 0.85 - index * 0.02), sourceQuality: 0.55, crochetConfidence: 0.65, freshness: item.publishedDate ? 0.6 : 0.3, engagement: 0.25, completeness: 0.5 }
  })) };
}
