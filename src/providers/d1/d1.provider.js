...
export async function searchInternal({ query, limit, offset, signal }) {
  const result = await queryD1(`SELECT id, type, source, title, description, url, image_url, author, language, published_at, tags_json, source_quality FROM SEARCH_RESULTS WHERE status = 'active' AND (title LIKE ? OR description LIKE ?) ORDER BY source_quality DESC, published_at DESC LIMIT ? OFFSET ?`, [`%${query}%`, `%${query}%`, limit, offset], { signal });
